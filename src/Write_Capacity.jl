function write_capacity_files(datadir::AbstractString)
    ...
    # ─────────────────────────────────────────────────────────────────────
    h2_list = Vector{Tuple{Int, String, Float64, Float64, Float64, Float64}}()
    # Generators
    for h in H
        header = String(hsc_gen[h, :resource])
        startv = Float64(hsc_gen[h, :existing_cap_tonne_p_hr])
        caprep = Float64(hsc_gen[h, :rep_capacity])
        newv   = value(vNewH2GenCap[h]) * caprep
        retv   = value(vRetH2GenCap[h]) * caprep
        totv   = value(eTotH2GenCap[h])               # use model expression
        push!(h2_list, (h, header, startv, newv, retv, totv))
    end

    # Storages
    for q in Q
        header = String(hsc_gen[q, :resource])
        startv = Float64(hsc_gen[q, :existing_cap_tonne])
        caprep = Float64(hsc_gen[q, :rep_capacity])
        newv   = value(vNewH2StoCap[q]) * caprep
        retv   = value(vRetH2StoCap[q]) * caprep
        totv   = value(eTotH2StoCap[q])
        push!(h2_list, (q, header, startv, newv, retv, totv))
    end

    # Storage compressors
    for q in Q
        header = String(hsc_gen[q, :resource]) * "_Compressor"
        startv = Float64(hsc_gen[q, :existing_cap_comp_tonne_hr])
        newv   = value(vNewH2StoCompCap[q])
        retv   = value(vRetH2StoCompCap[q])
        totv   = value(eTotH2StoCompCap[q])
        push!(h2_list, (q, header, startv, newv, retv, totv))
    end

    # ---- Build the frame and write ----
    Nh = length(h2_list)
    colnames_h = ["Attribute"; [item[2] for item in h2_list]...]  # item[2] is header::String
    names_sym_h = Symbol.(colnames_h)

    mat_h = Matrix{Any}(undef, 5, Nh+1); mat_h .= missing
    ...
    for (j, (r, header, startv, newv, retv, totv)) in enumerate(h2_list)
        col = Symbol(header)
        z   = hsc_gen[r, :zone]
        df_h[1, col] = z
        df_h[2, col] = startv

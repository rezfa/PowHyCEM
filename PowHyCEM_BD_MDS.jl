...
#function run_benders(datadir = joinpath("/Users/rez/Documents/Engineering/Coding/Julia/PowHyCEM/Input_Data"))
 ...
 # ---------- 1.  raw tables ------------------------------------------------
    ...
    for w in W
        ...
        @expression(SP_models[w], SlackCost, sum((slack_end[s])*100000000 for s in Q))
        ...
        @constraint(SP_models[w], cH2StoBalance[s in Q, t in 2:length(T)-1], vH2StoSOC[s,t] == (1-hsc_gen[s, :etta_self_dis])*vH2StoSOC[s,t-1] + vH2StoCha[s,t]*hsc_gen[s,:etta_cha] - (1/hsc_gen[s,:etta_dis])*vH2StoDis[s,t])
        @constraint(SP_models[w], cH2StoBalanceFirst[s in Q], vH2StoSOC[s,1] == (1-hsc_gen[s, :etta_self_dis])*eH2SOCFirst[s] + vH2StoCha[s,1]*hsc_gen[s,:etta_cha] - (1/hsc_gen[s,:etta_dis])*vH2StoDis[s,1])
        ...
        @constraint(SP_models[w], cH2StoSOCLast[s in Q], eH2SOCLast[s] == vH2StoSOC[s,168] + slack_end[s])

 ...
 # ---------- 1.  raw tables ------------------------------------------------
    ...
    for k in 1:max_iter
        ...
        if reg_con !== nothing
            ...
            delete(MP, reg_con)
            # reset Gurobi back to its default solve method
        ...
        println(" → Regularized MP objective = ", round(objective_value(MP), digits=2), " (not used for LB/UB, interior solution for next iteration)")
        push!(iters, k)
        push!(LB_hist, LB  / 1e6)   
        push!(UB_hist, UB  / 1e6)

...
#function run_benders(datadir = joinpath("/Users/rez/Documents/Engineering/Coding/Julia/PowHyCEM/Input_Data"))
 ...
 # ---------- 1.  raw tables ------------------------------------------------
    ...
    @variable(SP, eAvailPowGenCap[g in G]>=0)
    @variable(SP, eAvailPowGenUnit[g in G_ther]>=0)
    @variable(SP, eAvailPowStoCap[s in S]>=0)
    @variable(SP, eAvailPowTraCap[l in L]>=0)
    @variable(SP, eAvailH2GenCap[h in H]>=0)
    @variable(SP, eAvailH2GenUnit[h in H_ther]>=0)
    @variable(SP, eAvailH2StoCap[s in Q]>=0)
    @variable(SP, eAvailH2Pipe[i in I]>=0)
    @variable(SP, eAvailH2PipeCompCap[i in I]>=0)
    @variable(SP, eMaxEmissionByWeek[z in Z, w in W]>=0)
    ...
    @expression(SP, eZonalEmissionByWeek[z in Z, w in W], sum(vPowGen[g,w,t]*pow_gen[g, :heat_rate_mmbtu_per_yr]*CO2_content[pow_gen[g, :fuel]]*(pow_gen[g, :zone] == z ? 1 : 0) for g in G, t in T) + 
    ...
    @constraint(SP, cH2GenRampDn[g in H_dis, w in W, t in 2:length(T)], vH2Gen[g,w,t-1]-vH2Gen[g,w,t] .- hsc_gen[g, :ramp_down_percentage] * eAvailH2GenCap[g]<= 0)
    @constraint(SP, cH2GenRampDnFirst[g in H_dis, w in W], vH2GenFirst[g,w]-vH2Gen[g,w,1] .- hsc_gen[g, :ramp_down_percentage] * eAvailH2GenCap[g]<= 0)
    ...
    #Emission constraint
    @constraint(SP, cZonalEmissionCapByWeek[z in Z, w in W], eZonalEmissionByWeek[z,w] .- vExtraEmissionByZone[z,w] .- eMaxEmissionByWeek[z,w]<= 0)
    
    # for power side
    pi_cMaxPowGen      = Dict{eltype(G_ren),Float64}()
    pi_cPowOnlineUnits = Dict{eltype(G_ther),Float64}()
    pi_cPowGenRamp     = Dict{eltype(G_ren),Float64}()
    pi_cPowStoSOC      = Dict{eltype(S),  Float64}()
    pi_cMaxPowFlow     = Dict{eltype(L),  Float64}()

    # for hydrogen side
    pi_cMaxH2GenVar    = Dict{eltype(H_dis),Float64}()
    pi_cH2OnlineUnits  = Dict{eltype(H_ther),Float64}()
    pi_cH2GenRamp      = Dict{eltype(H_dis),Float64}()
    pi_cH2StoSOC       = Dict{eltype(Q),Float64}()
    pi_cH2PipeFlow     = Dict{eltype(I),Float64}()
    pi_cH2PipeFlowComp = Dict{eltype(I),Float64}()      
    pi_cZonalEmissionCapByWeek = Dict{eltype(Z),Float64}()
    

    #while (gap > tolerence) && (iteration <= max_iter)

        iteration += 1
        println("────────────────────────────────────────")
        println(" BENDERS ITERATION $iteration")
        println("────────────────────────────────────────")

        optimize!(MP)
        println("MP status: ", termination_status(MP))
        @assert termination_status(MP) == MOI.OPTIMAL

        LB = objective_value(MP)
        println(" → MP objective (LB) = ", round(LB, digits=2))

        # ---- SP Expressions ---- #
        for g in G
            avail = pow_gen[g,:existing_cap] + pow_gen[g,:rep_capacity] * (value(vNewPowGenCap[g]) - value(vRetPowGenCap[g]))
            fix(eAvailPowGenCap[g], avail; force = true)
        end
        for g in G_ther
            avail = pow_gen[g,:num_units] + value(vNewPowGenCap[g]) - value(vRetPowGenCap[g])
            fix(eAvailPowGenUnit[g], avail; force = true)
        end
        for s in S
            avail = pow_gen[s,:existing_cap] + pow_gen[s,:rep_capacity] * (value(vNewPowStoCap[s]) - value(vRetPowStoCap[s]))
            fix(eAvailPowStoCap[s], avail; force = true)
        end
        for l in L
            avail = pow_lines[l, :existing_transmission_cap_mw] + value(vNewPowTraCap[l])
            fix(eAvailPowTraCap[l], avail; force = true)
        end

        for h in H
            avail = hsc_gen[h, :existing_cap_tonne_p_hr] + hsc_gen[h, :rep_capacity] * (value(vNewH2GenCap[h]) - value(vRetH2GenCap[h]))
            fix(eAvailH2GenCap[h], avail; force = true)
        end
        for h in H_ther
            avail = hsc_gen[h, :num_units] + value(vNewH2GenCap[h]) - value(vRetH2GenCap[h])
            fix(eAvailH2GenUnit[h], avail; force = true)
        end
        for s in Q
            avail = hsc_gen[s, :existing_cap_tonne] + hsc_gen[s, :rep_capacity] * (value(vNewH2StoCap[s]) - value(vRetH2StoCap[s]))
            fix(eAvailH2StoCap[s], avail; force = true)
        end
        for i in I
            avail = hsc_pipelines[i, :existing_num_pipes] + (value(vNewH2Pipe[i]) - value(vRetH2Pipe[i]))
            fix(eAvailH2Pipe[i], avail; force = true)
        end
        for i in I
            avail = hsc_pipelines[i, :existing_comp_cap_tonne_hr] + value(vNewH2PipeCompCap[i]) - value(vRetH2PipeCompCap[i])
            fix(eAvailH2Pipe[i], avail; force = true)
        end
        for z in Z, w in W
            fix(eMaxEmissionByWeek[z,w], value(MP[:vMaxEmissionByWeek][z,w]); force = true)
        end
        
        optimize!(SP)
        println("SP status: ", termination_status(SP))
        sp_obj = objective_value(SP)
        println(" → SP objective         = ", round(sp_obj, digits=2))
        @assert termination_status(SP) == MOI.OPTIMAL
        ...
        for g in G_ren
            pi_cMaxPowGen[g] = sum(dual(cMaxPowGen[g,w,t])* pow_gen[g, :rep_capacity]*pow_gen_var[H_w[w][t], pow_gen[g, :resource]] for w in W, t in T)
        end
        for g in G_ther
            pi_cPowOnlineUnits[g] = sum((dual(cPowOnlineUnits[g,w,t])+dual(cPowStartLimits[g,w,t]))*pow_gen[g, :rep_capacity] for w in W, t in T)
        end
        for s in S
            pi_cPowStoSOC[s] = sum(dual(cPowStoSOCMax[s,w,t])*pow_gen[s, :rep_capacity]*pow_gen[s, :max_op_level] - dual(cPowStoSOCMin[s,w,t])*pow_gen[s, :rep_capacity]*pow_gen[s, :min_op_level] for w in W, t in 2:length(T))
        end
        for l in L
            pi_cMaxPowFlow[l] = sum(dual(cMaxPowFlowOut[l,w,t])-dual(cMaxPowFlowIn[l,w,t]) for w in W, t in T)
        end
        for h in H_dis
            pi_cMaxH2GenVar[h] = sum((dual(cMaxH2GenVar[h,w,t])-dual(cMinH2GenVar[h,w,t]))*hsc_gen[h, :max_op_level]*hsc_gen_var[H_w[w][t], hsc_gen[h, :resource]]*hsc_gen[h, :rep_capacity] for w in W, t in T)
        end
        for h in H_ther
            pi_cH2OnlineUnits[h] = sum((dual(cH2OnlineUnits[h,w,t])+dual(cH2StartLimits[h,w,t]))*hsc_gen[h, :rep_capacity] for w in W, t in T)
        end
        for s in Q
            pi_cH2StoSOC[s] = sum((dual(cMaxH2StoSOC[s,w,t])*hsc_gen[s,:h2stor_max_level] - dual(cMinH2StoSOC[s,w,t])*hsc_gen[s,:h2stor_min_level])*hsc_gen[s, :rep_capacity] for w in W, t in 2:length(T))
        end
        for i in I
            pi_cH2PipeFlow[i] = sum((dual(cMaxH2PipeFlowOut[i,w,t])-dual(cMaxH2PipeFlowIn[i,w,t]))*hsc_pipelines[i, :max_op_level] for w in W, t in T)
            pi_cH2PipeFlowComp[i] = sum(dual(cMaxH2PipeFlowOutComp[i,w,t]) - dual(cMaxH2PipeFlowInComp[i,w,t]) for w in W, t in T)
        end
        for z in Z
            pi_cZonalEmissionCapByWeek[z] = sum(dual(cZonalEmissionCapByWeek[z,w]) for w in W)
        end
        println(pi_cPowOnlineUnits)
        ...
        @constraint(MP,
            ...
            sum(pi_cPowStoSOC[s]*(vNewPowStoCap[s]-vRetPowStoCap[s] - value(MP[:vNewPowStoCap][s]) + value(MP[:vRetPowStoCap][s])) for s in S) +
            ...
            sum(pi_cH2StoSOC[s]*(vNewH2StoCap[s]-vRetH2StoCap[s] - value(MP[:vNewH2StoCap][s]) + value(MP[:vRetH2StoCap][s])) for s in Q) +
            ...
            sum(pi_cZonalEmissionCapByWeek[z]*(vMaxEmissionByWeek[z,w] - value(MP[:vMaxEmissionByWeek][z,w])) for z in Z, w in W) +
            sum(dual(cPowNSD[z,w])*vPowNSD[z,w,t] for z in Z, w in W, t in T) +
        )
            
        
#    end
    println("Done:  iterations = $iteration, final gap = ", round(gap, digits=2))
#end

...
while (gap > tolerence*UB) && (iteration <= max_iter)
    ...
    if termination_status(SP) == MOI.OPTIMAL
        ...
        pi_cPowStoSOC[s in S] = sum(dual(cPowStoSOCMax[s,w,t])*pow_gen[s, :rep_capacity]*pow_gen[s, :max_op_level] - dual(cPowStoSOCMin[s,w,t])*pow_gen[s, :rep_capacity]*pow_gen[s, :min_op_level] for w in W, t in 2:length(T))
        ...
        @constraint(MP, BD_Opt_Cut,
            ...
            sum(pi_cPowStoSOC[s]*(vNewPowStoCap[s]-vRetPowStoCap[s] - value(MP[:vNewPowGenCap][s]) + value(MP[:vRetPowStoCap][s])) for s in S) +
            ...
            sum(pi_cH2StoSOC[s]*(vNewH2StoCap[s]-vRetH2StoCap[s] - value(MP[:vNewH2GenCap][s]) + value(MP[:vRetH2StoCap][s])) for s in Q) +

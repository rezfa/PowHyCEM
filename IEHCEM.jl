...
@variable(CEM, vH2Flow[i in I, t in T]>=0)
...
#@expression(CEM, eH2FlowPos[i in I, t in T], vH2Flow[i,t],*(H2_Network[i,z]==1 ? 1 : 0))
...
@expression(CEM, ePowDemandHSC[t in T, z in Z], sum(hsc_gen[h, :pow_demand_mwh_p_tonne]*vH2Gen[h,t]*(hsc_gen[h, :zone]==z ? 1 : 0) for h in H) + 
                                                sum(hsc_gen[w, :pow_demand_mwh_p_tonne]*vH2StoCha[w,t]*(hsc_gen[w, :zone]==z ? 1 : 0) for w in W) + 
...
@expression(CEM, H2_D[t in T, z in Z], h2_demand[t,z] .+ ePowDemandHSC[t,z])

#Defining Objective Function

obj_pow = eCostPowGenInv .+ eCostPowStoInv .+ eCostPowTraInv .+ eCostPowGenVar .+ eCostPowStoVar .+ eCostPowNSD
obj_hsc = eCostH2GenInv .+ eCostH2StoInv .+ eCostH2TraInv .+ eCostH2GenVar .+ eCostH2GenStart .+ eCostH2NSD .+eCostH2StoCompInv

@objective(CEM, Min, obj_pow .+ obj_hsc)
...
for g in G
  if pow_gen[g, :max_cap_mw]!= -1
    @constraint(CEM, eTotPowGenCap[g] .- pow_gen[g, :max_cap_mw]<= 0)
  end
end
...
for s in S
  if pow_gen[s, :max_cap_mwh]!= -1
    @constraint(CEM, eTotPowStoCap[s] .- pow_gen[s, :max_cap_mwh]<= 0)
  end
end
...
@constraint(CEM, cH2Balance[z in Z,t in T],
  sum((vH2Gen[h,t] .- eH2GenEvap[h,t])*(hsc_gen[h, :zone]==z ? 1 : 0) for h in H) .- sum(H2_Network[i,z]*vH2Flow[i,t] for i in I) + 
  sum((vH2StoDis[w,t] - vH2StoCha[w,t])*(hsc_gen[w, :zone]==z ? 1 : 0) for w in W) + vH2NSD[z,t] == H2_D[t,z]
)
...
for h in H
  if hsc_gen[h, :max_cap_tonne_p_hr] != -1
...
@constraint(CEM, cH2GenCommitCon[h in J, t in T[1:8759]], vH2GenCommit[h,t+1]-vH2GenCommit[h,t] == vH2GenStart[h,t+1]-vH2GenShut[h,t+1])
...
for w in W
  if hsc_gen[w, :max_cap_stor_tonne] != -1
...
@constraint(CEM, cMaxRetH2PipeNume[i in I], vRetH2Pipe[i] <= hsc_pipelines[i, :existing_num_pipes])
@constraint(CEM, cMinH2PipeFlow[i in I, t in T], hsc_pipelines[i, :max_flow_tonne_per_h_per_pipe]*hsc_pipelines[i, :min_op_level] <= vH2Flow[i,t])
@constraint(CEM, cMaxH2PipeFlow[i in I, t in T], vH2Flow[i,t] <= hsc_pipelines[i, :max_flow_tonne_per_h_per_pipe]*hsc_pipelines[i, :max_op_level]*eTotH2Pipe[i])
@constraint(CEM, cMaxRetH2PipeCompCap[i in I], vRetH2PipeCompCap[i]<=hsc_pipelines[i, :existing_comp_cap_tonne_hr])
@constraint(CEM, cMaxH2PipeFlowComp[i in I,t in T], vH2Flow[i,t] <= eTotH2PipeCompCap[i])
...
for g in G
  println("New Generation Capacity fo Generator", pow_gen[g, :Resource], "in Zone", pow_gen[g, :Zone], ": ", value(vNewPowGenCap[g]))
end   